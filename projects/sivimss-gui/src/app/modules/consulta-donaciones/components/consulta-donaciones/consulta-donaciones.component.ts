...
import { CATALOGOS_DONADO_POR, CATALOGOS_DUMMIES } from '../../../servicios-funerarios/constants/dummies'
...
export class ConsultaDonacionesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.velatorios = mapearArregloTipoDropdown(respuesta[0]?.datos,'NOM_VELATORIO', 'ID_VELATORIO',
    )
    this.niveles = mapearArregloTipoDropdown( respuesta[1]?.datos,'DES_NIVELOFICINA','ID_OFICINA',
    )
  }
  ...
  paginarPorFiltros(): void {
    ...
    this.consultaDonacionesService.buscarAtaudesPorFiltros(solicitudFiltros,this.numPaginaActual,this.cantElementosPorPagina,)
      .subscribe((respuesta) => {
